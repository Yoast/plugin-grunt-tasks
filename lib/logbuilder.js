...
class ChangelogBuilder {
	...
	*/
	 ...
	 * Parse changelog lines parseChancelogLines special for yoast-cli generated file.
	 * @param {string} changelogIn changelos to be parsed .
	 * @param {boolean} substract defualt is false
	 * @param {string} specific only parse entry's of this type
	 * @param {boolean} trimspecific defualt false
	 * @returns {null} does not return
	 */
	parseYoastCliGeneratedChangelogPackageItemsOnly( changelogIn, substract = false, specific = "", trimspecific = false ) {
		...
		if ( trimspecific ) {
			changelogIn = changelogIn.replace( new RegExp( specific, "gm" ),
				""
			);
			// eslint-disable-next-line no-control-regex
			changelogIn = changelogIn.replace( new RegExp( "\n\\*\\W+", "gm" ),
	...
	get cleanChangelog() {
		...
		const usepackageformat = false;
		return this.changelogformatter( showuserfacing, usepackageformat );
	}
	/**
	 * Returns the class generated changelog
	 ...
	 */
	 get packageChangelog() {
		const showuserfacing = true;
		const usepackageformat = true;
		return this.changelogformatter( showuserfacing, usepackageformat, true );
	}
	// This shoulk be a private function
	/**
	 *
	 * @param {string} key key to format
	 * @param {boolean} usepackageformat defualt = false
	 * @returns {string} formated line
	 */
	linesformatter( key, usepackageformat = false ) {
		var lines = "";
		var title = key;
		if ( usepackageformat ) {
			title = "### " + key.replace( ":", "" );
	 ...
	 * @returns {string} changelog qa
	 */
	changelogformatter( shownonuserfacing = false, usepackageformat = false, removeGitLinks = false ) {
		// Start with enhancements
		var newlines = this.linesformatter( "Enhancements:", usepackageformat );
		// Add bugfixes
		newlines = newlines + this.linesformatter( "Bugfixes:", usepackageformat );
		// Add undefined
		// eslint-disable-next-line no-unused-vars
		this.ChangelogMap.forEach( function( _value, key, _map ) {
			if ( ! ( key === "Enhancements:" || key === "Bugfixes:" || ( key === "Non user facing:"   ) || key === "Other:" ) ) {
				newlines = newlines + this.linesformatter( key, usepackageformat );
			}
		}, this );
		// Add other
		newlines = newlines + this.linesformatter( "Other:", usepackageformat );
		if ( removeGitLinks ) {
		...
		if (  shownonuserfacing )  {
			newlines = newlines + this.linesformatter( "Non user facing:", usepackageformat );
		}
		return newlines;
	}
}
