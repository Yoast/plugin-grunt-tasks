	...
	*/
	...
	addLinesPerHeader( value ) {
		...
		const key =  this.packageTypeChangelog
			?  `${value.match( mykeyregexp )}`.replace( "### ", "" )
	 ...
	 * @param {string} specific only parse entry's of this type or remove these (depens on next value)
	 * @param {boolean} trimspecific default is false remove the specific items.
	 * @returns {null} does not return
	 */
	parseYoastCliGeneratedChangelogPackageItemsOnly( changelogIn, substract = false, specific = "", trimspecific = false ) {
		...
		if ( trimspecific ) {
			changelogIn = changelogIn.replace( new RegExp( escapeRegExp( specific ), "gm" ),
				""
			);
			// eslint-disable-next-line no-control-regex
			changelogIn = changelogIn.replace( new RegExp( "\n\\*\\W+", "gm" ),
	 ...
	 */
	 get packageChangelog() {
		const showuserfacing = true;
		return this.changelogformatter( showuserfacing,  true );
	}
